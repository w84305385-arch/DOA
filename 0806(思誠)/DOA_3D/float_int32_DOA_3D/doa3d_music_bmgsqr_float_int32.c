...
int search_count = 0;
//----------------------------------------------------------------
void doa3d_music_bmgsqr_cpu_float_int32_test(float *Rx_sig_re, float *Rx_sig_im, PhysicalParameters phys, RxParameters rx, TxParameters tx)
{
    //-------------------------------------------------------------------
    ...
    //print_complex_matrix(Rx_sig_re, Rx_sig_im, rx.Rx_M_x * rx.Rx_M_y, rx.nd);
    ...
    //     printf("Eigenvalue #%d = %f + %fi\n", i, De_re[i+i*Rx_M], De_im[i+i*Rx_M]);
    // }
    //---------------------------------------------------------------
    ...
    // Search angle theta 
    search_start_theta[0] = 0; // Starting angle of search
    ...
    // Search angle phi
    search_start_phi[0] = -60; // Starting angle of search
    ...
    for (int i = 0; i < search_len_phi; ++i){ 
        ...
        //printf("search_phi_rad[%d] = %f\n", i, search_phi_rad[i]);
    }
    
    // Calculate Spatial Spectrum and Peak Search
    float *S_MUSIC_dB = (float *)malloc(search_len_theta*search_len_phi * sizeof(float));
    ...
    position_theta[0]=0;
    position_phi[0]=0;
    find_spatial_spectrum_peaks_3D(S_MUSIC_dB, position_theta, position_phi, search_len_theta, search_len_phi, len_t_angle);
    ...
    //-------------------------------------------------------------------
    // save_Spectrum_to_csv("../float_DOA_3D/MUSIC_spectrum_dB/S_MUSIC_dB.csv", S_MUSIC_dB, search_len_theta*search_len_phi);
    //-------------------------------------------------------------------
    // free memory
    free(R_xx_re);
    free(R_xx_im);
...
int main()
{
    ...
    int M = 8;
    float angle_theta[100] = {45.0, 15, 15, 60.0, 70, 45, 58}; // elevation
    float angle_phi[100]    = {0.0, 16, 14, 45.0, 0, 45, 58}; // azimuth
    ...
    //=================== MUSIC Algorithm =================================
    doa3d_music_bmgsqr_cpu_float_int32_test(Rx_sig_re, Rx_sig_im, phys, rx, tx);
