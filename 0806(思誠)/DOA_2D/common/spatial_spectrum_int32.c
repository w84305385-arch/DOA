...
#define Q_SCALE  ((float)(1 << Q_SHIFT))
//--------------------
#define PI acos(-1)
#define AVX 16            
//--------------------
...
void float_to_q_int32_avx512(int32_t *restrict out_re,
                             ...
                             size_t      len)
{
    // Broadcast 2^Q_SHIFT as a 512-bit constant
    const __m512 scale_ps = _mm512_set1_ps((float)(1 << Q_SHIFT));

    size_t i         = 0;
    const size_t vec = len & ~15UL;     // Vectorized length (process 16 floats per iteration)

    // Main loop: process 16 elements per iteration
    for (; i < vec; i += 16) {
        // Load 16 float values from real and imaginary parts
        __m512 v_re = _mm512_loadu_ps(in_re + i);
        __m512 v_im = _mm512_loadu_ps(in_im + i);

        // Scale the floats by 2^Q_SHIFT to convert to fixed-point
        v_re = _mm512_mul_ps(v_re, scale_ps);
        v_im = _mm512_mul_ps(v_im, scale_ps);

        // Convert to int32 with rounding using _mm512_cvtps_epi32
    ...
    // Tail processing: handle the remaining elements (less than 16) using masks
    if (i < len) {
...
void q_int32_to_float_avx512(float   *restrict out_re,
                             ...
                             size_t      len)
{
    // Create a constant vector of 1 / (2^Q_SHIFT)
    const float  inv_scale = 1.0f / (float)(1 << Q_SHIFT);
    const __m512 inv_ps    = _mm512_set1_ps(inv_scale);

    size_t i         = 0;
    const size_t vec = len & ~15UL;   // Align to 16 elements per vector

    // Main loop: process 16 complex elements per iteration
    for (; i < vec; i += 16) {
        // Load 16 int32 values and convert to float
        __m512 v_re = _mm512_cvtepi32_ps(_mm512_loadu_si512(in_re + i));
        __m512 v_im = _mm512_cvtepi32_ps(_mm512_loadu_si512(in_im + i));

        // Multiply by 1 / (2^Q_SHIFT) to recover floating-point values
    ...
    // Handle remaining elements (<16) using masks
    if (i < len) {
        const __mmask16 tail = (1u << (len - i)) - 1u;
...
void calculate_spatial_spectrum_int32(
...
)
{
    ...
    float *S_spatial_spectrum_re = (float *)malloc(search_len_theta * sizeof(float) + AVX * sizeof(float));
    float *S_spatial_spectrum_im = (float *)malloc(search_len_theta * sizeof(float) + AVX * sizeof(float));
    ...
    for (int elevation = 0; elevation < search_len_theta; ++elevation){//theta
        ...
        // --------------------------------------
        spatial_spectrum_value_int32(a_vector_re_int32, a_vector_im_int32, search_const->Pn_re, search_const->Pn_im, Rx_M, &S_spatial_spectrum_re[elevation], &S_spatial_spectrum_im[elevation]);
    ...
    for (int elevation = 0; elevation < search_len_theta; ++elevation){//theta
            S_spatial_spectrum_im[elevation] = 0.0;
            spatial_spectrum_value_dB[elevation] = cpp_20log_abs(&S_spatial_spectrum_re[elevation], &S_spatial_spectrum_im[elevation]);
    }
