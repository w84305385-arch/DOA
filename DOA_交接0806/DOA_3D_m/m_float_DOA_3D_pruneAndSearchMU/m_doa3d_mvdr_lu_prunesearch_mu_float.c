...
//----------------------------------------------------------------
void m_doa3d_mvdr_lu_pns_mu_cpu_float_test(float *Rx_sig_re, float *Rx_sig_im, PhysicalParameters phys, RxParameters rx, TxParameters tx){
    //-------------------------------------------------------------------
    // Time parameter initializse
    ...
    struct timeval time_Rxx_inv_start, time_Rxx_inv_end, time_Rxx_inv_diff; // time initial
    ...
    for (int i = 0; i < peak_count0; ++i) {
        ...
        addThreadTask(prune_and_search_worker_PAPR_MVDR, mvdr);
    }
    ...
    timersub(&time_subRxx_end, &time_subRxx_start, &time_subRxx_diff);           // calculate subarray Rxx
...
int main(){
    ...
    int M = 8;
    float angle_theta[100] = { 15, 30, 10.0, 80.0, 70, 45, 58}; // elevation
    float angle_phi[100]    = { 15, 29, -35.0, 45.0, 0, 45, 58}; // azimuth
    ...
    //=================== MVDR Algorithm =================================
    m_doa3d_mvdr_lu_pns_mu_cpu_float_test(Rx_sig_re, Rx_sig_im, phys, rx, tx);
