...
void m_qr(float *A_re, float *A_im, float *Q_re, float *Q_im, float *R_re, float *R_im, int row, int col)
{
    // uesd memory pool
    ...
    for (int i = 0; i < col; ++i)
    {
        ...
        if(i<col-1)//i=0,1,2進////i=0,1,2,3,4,5,6進
        {
            ...
            //printf("proj_vector\n");
            //print_complex_matrix(proj_vector_re, proj_vector_im, 1, col-(i+1)); 
            m_complex_matrix_multiplication_row_block_parallel( Q_col_re, Q_col_im, proj_vector_re, proj_vector_im, Q_sub_re, Q_sub_im, row, 1, col-(i+1), num_row_blocks);// row*col-(i+1)
...
void m_BMGS_qr(float *A_re, float *A_im, float *Q_re, float *Q_im, float *R_re, float *R_im, int row, int col, int seg, float *total_QR)
{
    struct timeval time_qr_start, time_qr_end, time_qr_diff; // time initial
    ...
    for(int k = 0; k < seg; k++) {
        ...
        for(; j < k; j++) {
            ...
            m_complex_matrix_multiplication_row_block_parallel(Q_pre_re, Q_pre_im, A_block_re, A_block_im, R_temp_re + (k + j) * Block_col * Block_col, R_temp_im + (k + j) * Block_col * Block_col, Block_col, row, Block_col, num_row_blocks);
            ...
            m_complex_matrix_multiplication_row_block_parallel(Q_pre_re, Q_pre_im, R_temp_re + (k + j) * Block_col * Block_col, R_temp_im + (k + j) * Block_col * Block_col, temp_re, temp_im, row, Block_col, Block_col, num_row_blocks);
        ...
        gettimeofday(&time_qr_start, NULL);
        m_qr(A_block_re, A_block_im, Q_seg_re, Q_seg_im, R_seg_re, R_seg_im, row, Block_col);
        gettimeofday(&time_qr_end, NULL);
        timersub(&time_qr_end, &time_qr_start, &time_qr_diff);
        *total_QR += time_qr_diff.tv_sec * 1000000 + time_qr_diff.tv_usec;
        //printf("total_QR = %f\n", *total_QR);
...
void m_eigen_upper_triangular(float *A_re, float *A_im, float *eigenvalue_re, float *eigenvalue_im, float *eigenvector_re, float *eigenvector_im, int row, int col)
{
    //---------------------------------------------------------------
...
void m_eigen_BMGS(float *A_re, float *A_im, float *Ve_re, float *Ve_im, float *De_re, float *De_im, int row, int col, int iter, int seg, float *BMGS_QR_time, float *QR_time)
{
    ...
    *QR_time = 0.0;
    ...
    for (int i = 0; i < iter; ++i)
    {
        ...
        //print_complex_matrix(R_re, R_im, row, col);
        //------------------------------------QR--------------------------------------------
        m_BMGS_qr(A_re, A_im, Q_re, Q_im, R_re, R_im, row, col, seg, QR_time);
        ...
        //------------------------------------------------------------------------
        m_complex_matrix_multiplication_row_block_parallel(R_re, R_im, Q_re, Q_im, A_re, A_im, row, row, col, num_row_blocks);
        //---------------------------------------------------------------
        // Q_total = Q_total * Q
        m_complex_matrix_multiplication_row_block_parallel(Q_total_re, Q_total_im, Q_re, Q_im, Q_temp_re, Q_temp_im, row, row, col, num_row_blocks);
    ...
    *BMGS_QR_time = time_QR;
    ...
    //---------------------------------------------------------------
    m_eigen_upper_triangular(A_re, A_im, YY0_re, YY0_im, XX0_re, XX0_im, row, col);
...
void m_matrix_inverse_eigen(float *Ve_re, float *Ve_im, float *De_re, float *De_im, float *Pn_re, float *Pn_im, int Rx_M){
    ...
    m_complex_matrix_multiplication_row_block_parallel(Ve_re, Ve_im, De_re, De_im, R_xx_inv_1_re, R_xx_inv_1_im, Rx_M, Rx_M, Rx_M, num_row_blocks);
    complex_matrix_conjugate_transpose(Ve_re, Ve_im, Rx_M, Rx_M);
    m_complex_matrix_multiplication_row_block_parallel(R_xx_inv_1_re, R_xx_inv_1_im, Ve_re, Ve_im, Pn_re, Pn_im, Rx_M, Rx_M, Rx_M, num_row_blocks);
    free(R_xx_inv_1_re);
    free(R_xx_inv_1_im);
    free(De_result);
}
