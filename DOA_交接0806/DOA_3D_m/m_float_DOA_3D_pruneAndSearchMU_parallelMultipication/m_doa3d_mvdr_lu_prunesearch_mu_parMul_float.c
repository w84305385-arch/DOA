...
//----------------------------------------------------------------
void m_doa3d_mvdr_lu_pns_mu_parMul_cpu_float_test(float *Rx_sig_re, float *Rx_sig_im, PhysicalParameters phys, RxParameters rx, TxParameters tx)
{
    //-------------------------------------------------------------------
    ...
    for (int k = 0; k < search_len_theta_prune0*search_len_phi_prune0; ++k)
            S_MVDR_dB_prune0[k] = S_MVDR_dB_prune0[k]/N_SUBARRAY;
    ...
    int peak_count0;
    printf("=================== search 0 ===================\n");
    float search0_PAPR_threshold = 5; // Adjustable threshold
    ...
    printf("=================== search 1 ===================\n");
    gettimeofday(&time_Spatial_Spectrum1_start, NULL);
    int search1_threshold = 15; // Adjustable threshold
    if(Rx_M == 576) {
        search1_threshold = 20;
    }
    ...
    for (int i = 0; i < peak_count0; ++i) {
        ...
        /* 配置整個擴充結構 */
        SearchThreadData_MVDR *mvdr = calloc(1, sizeof(*mvdr));
    
        /* === 填 base (舊欄位) ================= */
        mvdr->base.search_const_input_data = search_const;
        mvdr->base.theta_deg_prev = search_peune0[i].theta_deg;
        mvdr->base.phi_deg_prev   = search_peune0[i].phi_deg;
        mvdr->base.search_step_theta = search_step_theta[1];
        mvdr->base.search_step_phi   = search_step_phi[1];
        mvdr->base.PAPR_threshold    = search1_threshold;
        mvdr->base.search_threshold  = 0;
        mvdr->base.result_theta_deg  = malloc(25 * sizeof(float));
        mvdr->base.result_phi_deg    = malloc(25 * sizeof(float));
        mvdr->base.result_dB_value   = malloc(25 * sizeof(float));
    
        /* === 填擴充欄位 ======================= */
        if (Rx_M == 576) {
            mvdr->use_subarray        = 1;
            mvdr->N_SUBARRAY          = N_SUBARRAY;        // 16
            mvdr->search_const_sub_tpl = search_const_sub; // 4×4 模板
            mvdr->Pn_re_sub           = Pn_re_sub;
            mvdr->Pn_im_sub           = Pn_im_sub;
        } else {
            mvdr->use_subarray        = 0;
        }
    
        Search_td_ptrs1[i] = &mvdr->base;
        /* 加入 thread-pool；直接傳指標即可 */
        addThreadTask(prune_and_search_worker_PAPR_MVDR_sub, mvdr);
    }
    ...
    for (int i = 0; i < peak_count_total_1; i++) {
        ...
        addThreadTask(prune_and_search_worker_PAPR_MVDR, td);
    }
    ...
    timersub(&time_Rxx_inv_end, &time_Rxx_inv_start, &time_Rxx_inv_diff);           // calculate Eigen
    ...
    fprintf(file, "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f",
            time_Rxx / 1000,
            *BMGS_qr_time / 1000,
            time_Rxx_inv / 1000 - (*BMGS_qr_time / 1000),
            time_Pn / 1000,
            total_time_Spatial_Spectrum / 1000,
            total_time_Peak_Search / 1000,
            time_MVDR / 1000
            );// *qr_time / 1000,(*BMGS_qr_time / 1000) - (*qr_time / 1000),
    fclose(file);

    save_Spectrum_to_csv("data/3D_MVDR_dB.csv", S_MVDR_dB, search_len_theta*search_len_phi);
    */
    //-------------------------------------------------------------------
...
int main(){
    ...
    //=================== MVDR Algorithm =================================
    m_doa3d_mvdr_lu_pns_mu_parMul_cpu_float_test(Rx_sig_re, Rx_sig_im, phys, rx, tx);
