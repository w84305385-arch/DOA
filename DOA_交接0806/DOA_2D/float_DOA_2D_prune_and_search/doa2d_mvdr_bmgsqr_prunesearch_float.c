...
void doa2d_mvdr_bmgsqr_prunesearch_cpu_float_test(float *Rx_sig_re, float *Rx_sig_im, PhysicalParameters phys, RxParameters rx, TxParameters tx)
{
    //-------------------------------------------------------------------
    ...
    if (Rx_M > 8) {
        ...
        for (int sid = 0; sid < N_SUBARRAY; ++sid) {
            ...
            float BMGS_qr_time_sub, qr_time_sub;
            // Eigen-decomposition for subarray
            eigen_BMGS(Rxx_sub_re, Rxx_sub_im,
                       Ve_sub_re, Ve_sub_im,
                       De_sub_re, De_sub_im,
                       SUB_M, SUB_M,
                       qr_iter, 1,   // block = 1 is sufficient for small subarrays
                       &BMGS_qr_time_sub, &qr_time_sub);
    
            // Inverse of Rxx (subarray)
            Pn_re_sub[sid] = aligned_alloc(64, SUB_SIZE * sizeof(float));
            Pn_im_sub[sid] = aligned_alloc(64, SUB_SIZE * sizeof(float));
    
            matrix_inverse_eigen(Ve_sub_re, Ve_sub_im,
                                 De_sub_re, De_sub_im,
...
int main()
{
    ...
    //=================== MVDR Algorithm =================================
    doa2d_mvdr_bmgsqr_prunesearch_cpu_float_test(Rx_sig_re, Rx_sig_im, phys, rx, tx);
