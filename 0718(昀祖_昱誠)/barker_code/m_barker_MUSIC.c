// g++ -mavx512f -g -o m_barker_MUSIC m_barker_MUSIC.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_barker_MUSIC -t4
...
void *MUSIC_DOA_2A_CPU_test(void *struct_var)
{
    ...
    // timersub function
    //-------------------------------------------------------------------
    float  time_Eigen, time_Pn,  time_search, time_vet;                           // create float parameter in order to convert (us) to (ms)  // calculate AWGN
    ...
    float total_time = time_Eigen  + time_Pn + time_search  + time_vet;
...
void MUSIC_thread_4(int argc, char **argv)
{
    //-------------------------------------------------------------------
    int opt;
    int thread_num;
...
int main(int argc, char **argv){
    ...
    MUSIC_thread_4( argc, argv);//前四次
    //MUSIC 對上那次
    MUSIC_thread_4( argc, argv);//後四次
