//MVDR，方法2，對上那次&前4次、後4次做DOA
//4根再16根，4根用混合運算(1度)粗略搜索，16根用浮點運算(0.1度)精確搜索
// g++ -mavx512f -g -o hybrid_barker_method2_MVDR hybrid_barker_method2_MVDR.c -Wall -Wextra -std=c++14 math_func.a
// ./hybrid_barker_method2_MVDR
#define PI acos(-1)
...
void MVDR_DOA_2A_CPU_test(int M, int qr_iter,int hybrid_qr_iter, float *angle, int number_angle, float *result, int SNR, int *check_result)
{
    ...
    //---------------------------------------------------------------
    float *R_xx_inv_1_re4 = (float *)malloc(4 * 4 * sizeof(float));
    float *R_xx_inv_1_im4 = (float *)malloc(4 * 4 * sizeof(float));
    ...
    for (int i = 0; i < 4 * 4; i += (4 + 1))
    {
        ...
        {
            cpp_division3(&temp_real4, &temp_imag4, &De4_re[i], &De4_im[i]);
        }
    }
    complex_matrix_multiplication(Ve4_re, Ve4_im, De4_re, De4_im, R_xx_inv_1_re4, R_xx_inv_1_im4, 4, 4, 4);
    complex_matrix_conjugate_transpose(Ve4_re, Ve4_im, 4, 4);
    complex_matrix_multiplication(R_xx_inv_1_re4, R_xx_inv_1_im4, Ve4_re, Ve4_im, Pn4_re, Pn4_im, 4, 4, 4);
    // printf("----------R_xx------------\n");
