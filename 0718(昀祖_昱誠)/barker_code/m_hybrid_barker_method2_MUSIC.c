//MUSIC，方法2，對上那次&前4次、後4次做DOA，multi-user版本
//4根再16根，4根用混合運算(1度)粗略搜索，16根用浮點運算(0.1度)精確搜索
// g++ -mavx512f -g -o m_hybrid_barker_method2_MUSIC m_hybrid_barker_method2_MUSIC.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_hybrid_barker_method2_MUSIC -t4
#define PI acos(-1)
...
void *MUSIC_DOA_2A_CPU_test(void *struct_var)
{
    ...
    //---------------------------------------------------------------
    int *vet_noise4_re_i = (int *)malloc(4 * (4 - len_t_theta) * sizeof(int));
    int *vet_noise4_im_i = (int *)malloc(4 * (4 - len_t_theta) * sizeof(int));
    //---------------------------------------------------------------
    for (int i = 0; i < 4; ++i)
    {
        for (int j = len_t_theta; j < 4; ++j)
        {
            vet_noise4_re_i[i * (4 - len_t_theta) + j - len_t_theta] = Ve4_re[i * 4 + j];
            vet_noise4_im_i[i * (4 - len_t_theta) + j - len_t_theta] = Ve4_im[i * 4 + j];
            //printf("\t(%f,%f)\n", Ve_re[i * M + j], Ve_im[i * M + j]);
        }
    }
    for (int i = 0; i < 4; ++i)
    ...
    //---------------------------------------------------------------
    int *Pn4_re_i = (int *)calloc( 4 * 4, sizeof(int));
    int *Pn4_im_i = (int *)calloc( 4 * 4, sizeof(int));
    //---------------------------------------------------------------
    ...
    /*
    compute_Pn_i(Pn4_re_i, Pn4_im_i, vet_noise4_re_i, vet_noise4_im_i, 4, len_t_theta);
    for(int w=0;w<4*4;w++){
        Pn4_re[w]=Pn4_re_i[w];
        Pn4_im[w]=Pn4_im_i[w];
    }
    */
    ...
    // timersub function
    //-------------------------------------------------------------------
    float  time_Eigen,time_Eigen_4, time_Pn,  time_search, time_small_search, time_vet;                           
