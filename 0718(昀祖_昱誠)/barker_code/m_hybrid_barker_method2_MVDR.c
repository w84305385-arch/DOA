//MVDR，方法2，對上那次&前4次、後4次做DOA，multi-user版本
//4根再16根，4根用混合運算(1度)粗略搜索，16根用浮點運算(0.1度)精確搜索
// g++ -mavx512f -g -o m_hybrid_barker_method2_MVDR m_hybrid_barker_method2_MVDR.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_hybrid_barker_method2_MVDR -t4
#define PI acos(-1)
...
void *MVDR_DOA_2A_CPU_test(void *struct_var)
{
    ...
    int *check_result = mvdr_param->check_result;
    //-------------------------------------------------------------------
    printf("---------------\n");
    printf("--MVDR DOA--\n");
    printf("---------------\n");
    ...
    float *De4_result = (float *)malloc(4 * 4 * sizeof(float));
    //---------------------------------------------------------------
    ...
    hybrid_eigen(R_xx4_re, R_xx4_im, Ve4_re, Ve4_im, De4_re, De4_im, 4, 4, hybrid_qr_iter);
    ...
    //---------------------------------------------------------------
    float *R_xx4_inv_1_re = (float *)malloc(4 * 4 * sizeof(float));
    float *R_xx4_inv_1_im = (float *)malloc(4 * 4 * sizeof(float));
    ...
    float temp4_real = 1;
    float temp4_imag = 0;
    for (int i = 0; i < 4 * 4; i += (4 + 1))
    {
        cpp_abs(&De4_re[i], &De4_im[i], &De4_result[i]);
        if (abs(De4_result[i]) < 0.00000001)
        {
            De4_re[i] = 1000000;
            De4_im[i] = 0;
        }
        ...
        {
            cpp_division3(&temp4_real, &temp4_imag, &De4_re[i], &De4_im[i]);
        }
    }
    complex_matrix_multiplication(Ve4_re, Ve4_im, De4_re, De4_im, R_xx4_inv_1_re, R_xx4_inv_1_im, 4, 4, 4);
    complex_matrix_conjugate_transpose(Ve4_re, Ve4_im, 4, 4);
    complex_matrix_multiplication(R_xx4_inv_1_re, R_xx4_inv_1_im, Ve4_re, Ve4_im, Pn4_re, Pn4_im, 4, 4, 4);
    ...
    //---------------------------------------------------------------
    float *a_vector_re = (float *)malloc(4 * sizeof(float));
    ...
    for (int i = 0; i < len_dth; ++i)
    {
        ...
        compute_S_MUSIC(a_vector_re, a_vector_im, Pn4_re, Pn4_im, 4, &S_MUSIC_re[i], &S_MUSIC_im[i]);
    ...
    // timersub function
    //-------------------------------------------------------------------
    float time_AWGN, time_Eigen, time_Eigen_4, time_Pn,  time_search, time_small_search;                           // create float parameter in order to convert (us) to (ms)
    ...
    */
    ...
    float total_time =  time_Eigen + time_Eigen_4 + time_Pn  +time_search  + time_small_search;
...
void MVDR_thread_4(int argc, char **argv, int *check_result)
{
    //-------------------------------------------------------------------
    int opt;
    int thread_num;
    ...
    mvdr_param->SNR = snr;
    mvdr_param->check_result=check_result;
    //-------------------------------------------------------------------
    //=================== MUSIC Algorithm =================================
...
void *MVDR_DOA_2A_CPU_test1(void *struct_var)
{
    float time_MVDRre = 0.0;
    float timeMVDRre_start, timeMVDRre_end; // Total MUSIC Algorithm time
    float timeMVDR_test_start[4] = {0.0};
    ...
    // timersub function
    //-------------------------------------------------------------------
    float time_AWGN, time_Eigen, time_Pn,  time_search;                           // create float parameter in order to convert (us) to (ms)
    timersub(&time_AWGN_end, &time_AWGN_start, &time_AWGN_diff);    // calculate AWGN
    timersub(&time_Eigen_end, &time_Eigen_start, &time_Eigen_diff); // calculate Eigen
    timersub(&time_Pn_end, &time_Pn_start, &time_Pn_diff);          // calculate Pn
    timersub(&time_search_end, &time_search_start, &time_search_diff);
    ...
    */
    ...
    //printf("Total Pn time: \t\t%.3f(ms)\n", time_Pn / 1000);
    //printf("Total search time: \t\t%.3f(ms)\n", time_search / 1000);
    float total_time =  time_Eigen + time_Pn  +time_search  ;
    //printf(BLUE"Total time: \t%.3f(ms)\n"CLOSE, total_time / 1000);
...
void MVDR_thread_1(int argc, char **argv, int *check_result)
{
    //-------------------------------------------------------------------
    int opt;
    int thread_num;
    ...
    for (int i = 0; i < thread_num-3; i++)
    {
        timeMVDR_start[i] = clock();
        // printf("pthread_create[%d]\n", i);
        if (pthread_create(&th[i], NULL, &MVDR_DOA_2A_CPU_test1, mvdr_param) != 0)
        {
            perror("Failed to create thread\n");
            //return 1;
        }
...
int main(int argc, char **argv)
{
    ...
    MVDR_thread_4(argc, argv, &check_result);//前四次
    //printf("\n");
    //*
    for(int i=126;i<127;i++){
        ...
        //print_code(targetCode,CODE_LENGTH);
        checkpnCode(targetCode, target_sequence,&check_result);
        MVDR_thread_1(argc, argv, &check_result);
        //printf("\n");
    }
    ...
    MVDR_thread_4(argc, argv, &check_result);//後四次
