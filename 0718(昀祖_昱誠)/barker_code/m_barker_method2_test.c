//4根再16根 multi-thread
// g++ -mavx512f -g -o m_barker_method2_test m_barker_method2_test.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_barker_method2_test -t4
...
__attribute__((aligned(32))) float matC_Re_sum_single[1000000] = {0.0}; // Real
__attribute__((aligned(32))) float matC_Im_sum_single[1000000] = {0.0}; // Imaginary
__attribute__((aligned(32))) float matC_Real_single[1000000] = {0.0}; // re_C
__attribute__((aligned(32))) float matC_Imag_single[1000000] = {0.0}; // im_C
//-------------------------------------------------------------------
...
void matrix_transpose_single(float *matA_re, float *matA_im, int rowA, int colA)
{
    float *temp_re = (float *)malloc(colA * rowA * sizeof(float));
    float *temp_im = (float *)malloc(colA * rowA * sizeof(float));
    memcpy(temp_re, matA_re, (rowA * colA * sizeof(float)));
...
void complex_matrix_addition_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, int rowA, int colA)
{   
...
void complex_matrix_subtraction_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, int rowA, int colA)
{   
    if(rowA<16){
        for (int i = 0; i < rowA; ++i)
        {
...
void complex_matrix_multiplication_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, float *matC_re, float *matC_im, int rowA, int rowB, int colB)
{
    //-------------------------------------------------------------------
    // Parameter initialize
    struct timespec start, end;
    ...
    // Initialize Global variable array = 0
    //----------------------------------------------------------
    memset(matC_Re_sum_single, 0, rowA * colB * sizeof(double));
    memset(matC_Im_sum_single, 0, rowA * colB * sizeof(double));
    // memset(matA_re, 0, rowA * colB * sizeof(float));
    // memset(matA_im, 0, rowA * colB * sizeof(float));
    // memset(matB_re, 0, rowA * colB * sizeof(float));
    // memset(matB_im, 0, rowA * colB * sizeof(float));
    memset(matC_Real_single, 0, rowA * colB * sizeof(double));
    memset(matC_Imag_single, 0, rowA * colB * sizeof(double));
    //----------------------------------------------------------
    //----------------------------------------------------------
    ...
    for (int i = 0; i < rowA; i++)
    {
        for (int j = 0; j < colB; j++)
        {
            for (int k = 0; k < ((rowB - 1) / AVX) + 1; k++)
            {
                ...
                _mm512_storeu_ps(&matC_Real_single[(i * colB * rowB + j * rowB) + AVX * k], re_C); // store Re value
                _mm512_storeu_ps(&matC_Imag_single[(i * colB * rowB + j * rowB) + AVX * k], im_C); // store Im value
            }
        }
    }
    // gettimeofday(&end_multiply, NULL);
    ...
    for (int i = 0; i < rowA * colB; i++)
    {
        ...
        for (int a = 0; a < rowB; a++)
        {
            matC_Re_sum_single[i] += matC_Real_single[i * rowB + a];
            matC_Im_sum_single[i] += matC_Imag_single[i * rowB + a];
            // printf("matC_Re_sum[%d] = %.2f, ", i, matC_Re_sum[i]);
            //    printf("matC_Im_sum[%d] = %.0f, ", i * colB + a, matC_Im_sum[i]);
        }
        // printf(" \n");
    ...
    for (int i = 0; i < rowA * colB; i++)
    {
        // matC[i] = {matC_Real[i], matC_Imag[i]};
        //  matC[i] = matC_Imag[i];
        matC_re[i] = matC_Re_sum_single[i];
        matC_im[i] = matC_Im_sum_single[i];
    }
    //-------------------------------------------------------------------
    gettimeofday(&start_transpose, NULL);           // start
...
void complex_matrix_multiplication_noavx_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, float *matC_re, float *matC_im, int rowA, int rowB, int colB)
{
    memset(matC_re, 0, rowA * colB * sizeof(float));
    memset(matC_im, 0, rowA * colB * sizeof(float));
...
void complex_matrix_get_columns_single(float *matA_re, float *matA_im, float *matCol_re, float *matCol_im, int rowA, int colA, int colTarget)
{
    for (int i = 0; i < rowA; ++i)
    {
...
void complex_matrix_get_rows_single(float *matA_re, float *matA_im, float *matRow_re, float *matRow_im, int rowA, int colA, int rowTarget)
{
    for (int i = 0; i < colA; ++i)
    {
...
void complex_matrix_conjugate_transpose_single(float *matA_re, float *matA_im, int rowA, int colA)
{
    float *temp_re = (float *)malloc(colA * rowA * sizeof(float));
    float *temp_im = (float *)malloc(colA * rowA * sizeof(float));
    memcpy(temp_re, matA_re, (rowA * colA * sizeof(float)));
...
void complex_matrix_conjugate_transpose_multiplication_noavx_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, int rowA, int colA)
{
    float *temp_re = (float *)malloc(colA * rowA * sizeof(float));
    float *temp_im = (float *)malloc(colA * rowA * sizeof(float));
    memcpy(temp_re, matA_re, (rowA * colA * sizeof(float)));
    memcpy(temp_im, matA_im, (rowA * colA * sizeof(float)));
    complex_matrix_conjugate_transpose_single(temp_re, temp_im, rowA, colA);
    complex_matrix_multiplication_noavx_single(matA_re, matA_im, temp_re, temp_im, matB_re, matB_im, rowA, colA, rowA);

    free(temp_re);
    free(temp_im);
}
void complex_matrix_conjugate_transpose_multiplication_single(float *matA_re, float *matA_im, float *matB_re, float *matB_im, int rowA, int colA)
{
    float *temp_re = (float *)malloc(colA * rowA * sizeof(float));
    float *temp_im = (float *)malloc(colA * rowA * sizeof(float));
    memcpy(temp_re, matA_re, (rowA * colA * sizeof(float)));
    memcpy(temp_im, matA_im, (rowA * colA * sizeof(float)));
    complex_matrix_conjugate_transpose_single(temp_re, temp_im, rowA, colA);
    complex_matrix_multiplication_single(matA_re, matA_im, temp_re, temp_im, matB_re, matB_im, rowA, colA, rowA);

    free(temp_re);
    free(temp_im);
}
void compute_Pn_single(float *Pn_re, float *Pn_im, float *vet_noise_re, float *vet_noise_im, int M, int len_t_theta)
{
    //---------------------------------------------------------------
    float *vet_noise_temp_re = (float *)malloc(M * sizeof(float));
    float *vet_noise_temp_im = (float *)malloc(M * sizeof(float));
    ...
    for (int i = 0; i < M - len_t_theta; ++i)
    {
        complex_matrix_get_columns_single(vet_noise_re, vet_noise_im, vet_noise_temp_re, vet_noise_temp_im, M, M - len_t_theta, i);
        complex_matrix_conjugate_transpose_multiplication_noavx_single(vet_noise_temp_re, vet_noise_temp_im, Pn_temp_re, Pn_temp_im, M, 1);
        complex_matrix_addition_single(Pn_re, Pn_im, Pn_temp_re, Pn_temp_im, M, M);
    }
    free(vet_noise_temp_re);
    free(vet_noise_temp_im);
    free(Pn_temp_re);
...
void compute_S_MUSIC_single(float *a_vector_re, float *a_vector_im, float *Pn_re, float *Pn_im, int M, float *music_Real, float *music_Imag)
{
    //---------------------------------------------------------------
    float *Pn_a_vector_temp_re = (float *)malloc(M * sizeof(float));
    float *Pn_a_vector_temp_im = (float *)malloc(M * sizeof(float));
    ...
    // printf("temp = (%f,%f)\n", *temp_re, *temp_im);
    //---------------------------------------------------------------
    complex_matrix_multiplication_single(Pn_re, Pn_im, a_vector_re, a_vector_im, Pn_a_vector_temp_re, Pn_a_vector_temp_im, M, M, 1);
    complex_matrix_conjugate_transpose_single(a_vector_re, a_vector_im, M, 1);
    complex_matrix_multiplication_single(a_vector_re, a_vector_im, Pn_a_vector_temp_re, Pn_a_vector_temp_im, S_MUSIC_temp_re, S_MUSIC_temp_im, 1, M, 1);
    cpp_division2(1, 0, &S_MUSIC_temp_re[0], &S_MUSIC_temp_im[0], music_Real, music_Imag);

    //printf("music = (%.5f,%.5f)\n", *music_Real, *music_Real);
}
void qr_single(float *A_re, float *A_im, float *Q_re, float *Q_im, float *R_re, float *R_im, int row, int col)
{
    float *Q_col_temp_re = (float *)malloc(row * 1 * sizeof(float));
    float *Q_col_temp_im = (float *)malloc(row * 1 * sizeof(float));
    //--------------------------------------------------------------
    ...
    for (int i = 0; i < col; ++i)
    {
        ...
        complex_matrix_get_columns_single(Q_re, Q_im, Q_col_re, Q_col_im, row, col, i);
        complex_matrix_get_columns_single(Q_re, Q_im, Q_col_temp_re, Q_col_temp_im, row, col, i);
        //printf("Q_col歸一前,v(%d)\n",i);
        //print_complex_matrix(Q_col_re, Q_col_im, row, 1);
        /*
        if(i==0){    
        ...
        */
        complex_matrix_conjugate_transpose_single(Q_col_temp_re, Q_col_temp_im, row, 1);
        //printf("Q_col_temp^H\n");
        //print_complex_matrix(Q_col_temp_re, Q_col_temp_im,1 ,row );
        //printf("Q_col_temp\n");
        //print_complex_matrix(Q_col_temp_re, Q_col_temp_im, row, 1 );
        complex_matrix_conjugate_transpose_multiplication_single(Q_col_temp_re, Q_col_temp_im, power_val_re, power_val_im, 1, row); //v(i)長度的平方
        //printf("power_val開根號前\n");
        //print_complex_matrix(power_val_re, power_val_im, 1, 1);
        cpp_sqrt(&power_val_re[0], &power_val_im[0]);
        //printf("power_val開根號後:放到R對角線上\n");
        ...
        */
        ...
        //printf("歸一化後(調整過)q,(*a3:%d) \n",a3);
        //print_complex_matrix(Q_col_re, Q_col_im, row, 1);
        complex_matrix_conjugate_transpose_single( Q_col_re, Q_col_im, row, 1);//q(i) -> q(i)^H : row*1 -> 1*row(為了後續算內積)
        //printf("除完的Q_re,Q_im(%d):\n",i);
        //print_complex_matrix(Q_re, Q_im, row, col);
        //printf(BLUE"---\n"CLOSE);
        if(i<col-1)//i=0,1,2進////i=0,1,2,3,4,5,6進
        {
            ...
            //print_complex_matrix(Q_col_proj_re, Q_col_proj_im, row , col-(i+1));
            complex_matrix_multiplication_single(Q_col_re, Q_col_im, Q_col_proj_re, Q_col_proj_im, proj_vector_re, proj_vector_im, 1, row, col-(i+1)); //1*[col-(i+1)]//rij
            //printf("q*v:r:Q_col x Q_col_proj = proj_vector \n");
            //print_complex_matrix(proj_vector_re, proj_vector_im, 1, col-(i+1));
            ...
            //printf("R給右上:\n");
            //print_complex_matrix(R_re, R_im, row, col );
            complex_matrix_conjugate_transpose_single( Q_col_re, Q_col_im, 1, row);//q(i)^H -> q(i) : 1*row -> row*1
            //printf("Q_col(%d):\n",i);
            //print_complex_matrix(Q_col_re, Q_col_im, row, 1);
            ...
            //printf("proj_vector\n");
            //print_complex_matrix(proj_vector_re, proj_vector_im, 1, col-(i+1)); 
            complex_matrix_multiplication_single( Q_col_re, Q_col_im, proj_vector_re, proj_vector_im, Q_sub_re, Q_sub_im, row, 1, col-(i+1));// row*col-(i+1)
            //printf("Q_sub = Q_col x proj_vector\n");
            //print_complex_matrix(Q_sub_re, Q_sub_im,  row, col-(i+1));
            ...
            {    
                //printf(RED"進for分別減,i=%d,j=%d\n"CLOSE,i,j);
                complex_matrix_get_columns_single(Q_re, Q_im, vector_cur_re, vector_cur_im, row, col, j); //i=0、j=1時會取v(1) ; i=0、j=2時會取v(2)... -> row*1
                complex_matrix_get_columns_single(Q_sub_re, Q_sub_im, Q_col_re, Q_col_im, row, col-(i+1), j-(i+1));
                ...
                //printf("Q_col:\n");
                //print_complex_matrix(Q_col_re, Q_col_im,  row, 1);
                complex_matrix_subtraction_single(vector_cur_re, vector_cur_im, Q_col_re, Q_col_im, row, 1);
                ///* 配合讓proj_vector縮小別太小
                //if(i>0){
                    for(int m=0;m<col;m++){
                    vector_cur_re[m]=vector_cur_re[m]/a2;
...
void eigen_upper_triangular_single(float *A_re, float *A_im, float *eigenvalue_re, float *eigenvalue_im, float *eigenvector_re, float *eigenvector_im, int row, int col)
{
    //---------------------------------------------------------------
    float *vector_cur_re = (float *)malloc(row * 1 * sizeof(float));
    float *vector_cur_im = (float *)malloc(row * 1 * sizeof(float));
    ...
    for (int i = 0; i < col; ++i)
    {
        complex_matrix_get_columns_single(eigenvector_re, eigenvector_im, vector_cur_re, vector_cur_im, row, col, i);
        ...
        for (int j = i - 1; j > -1; --j)
        {
            ...
            {
                complex_matrix_get_rows_single(A_re, A_im, A_col_re, A_col_im, row, col, j);
                complex_matrix_multiplication_single(A_col_re, A_col_im, vector_cur_re, vector_cur_im, eigen_element_cur_re, eigen_element_cur_im, 1, row, 1);
                cpp_division(&eigen_element_cur_re[0], &eigen_element_cur_im[0], &diff_eigen_value_re, &diff_eigen_value_im);
            }
            vector_cur_re[j] = eigen_element_cur_re[0];
            vector_cur_im[j] = eigen_element_cur_im[0];
        }
        complex_matrix_conjugate_transpose_single(vector_cur_re, vector_cur_im, row, 1);
        complex_matrix_conjugate_transpose_multiplication_single(vector_cur_re, vector_cur_im, vector_cur_temp_re, vector_cur_temp_im, 1, row);
        cpp_sqrt(&vector_cur_temp_re[0], &vector_cur_temp_im[0]); // vector_cur_temp[0] = sqrt(vector_cur_temp[0]);
        complex_matrix_conjugate_transpose_single(vector_cur_re, vector_cur_im, 1, row);
        // Complex Division
        for (int m = 0; m < row; ++m)
        {
            cpp_division(&vector_cur_re[m], &vector_cur_im[m], &vector_cur_temp_re[0], &vector_cur_temp_im[0]);
...
void eigen_single(float *A_re, float *A_im, float *Ve_re, float *Ve_im, float *De_re, float *De_im, int row, int col, int iter)
{
    struct timeval start_QR, end_QR, diff_QR;
    float time_QR = 0.0;
    float *Q_re = (float *)calloc(row * col, sizeof(float));
    ...
    for (int i = 0; i < iter; ++i)
    {
        ...
        qr_single(A_re, A_im, Q_re, Q_im, R_re, R_im, row, col);
        ...
        // print_complex_matrix(R_re, R_im, row, col);
        //------------------------------------------------------------------------
        complex_matrix_multiplication_single(R_re, R_im, Q_re, Q_im, A_re, A_im, row, row, col);
        complex_matrix_multiplication_single(Q_temp_re, Q_temp_im, Q_re, Q_im, Q_temp_clone_re, Q_temp_clone_im, row, row, col);
        //---------------------------------------------------------------
        memcpy(Q_temp_re, Q_temp_clone_re, row * col * sizeof(float));
        memcpy(Q_temp_im, Q_temp_clone_im, row * col * sizeof(float));
    }
    ...
    float *XX0_im = (float *)calloc(row * col, sizeof(float));
    //---------------------------------------------------------------
    eigen_upper_triangular_single(A_re, A_im, YY0_re, YY0_im, XX0_re, XX0_im, row, col);
    memcpy(De_re, YY0_re, row * col * sizeof(float));
    memcpy(De_im, YY0_im, row * col * sizeof(float));
    complex_matrix_multiplication_single(Q_temp_re, Q_temp_im, XX0_re, XX0_im, Ve_re, Ve_im, row, row, col);
...
void MUSIC_DOA_2A_single(int M, int qr_iter,int hybrid_qr_iter, float *angle, int number_angle, float *result, int SNR, int *check_result)
{
    ...
    float *x_r4_im = (float *)malloc(4 * nd * sizeof(float));
    //---------------------------------------------------------------
    complex_matrix_multiplication_single(A_theta_re, A_theta_im, t_sig_re, t_sig_im, sig_co_re, sig_co_im, M, len_t_theta, nd);
    complex_matrix_multiplication_single(A_theta4_re, A_theta4_im, t_sig_re, t_sig_im, sig_co4_re, sig_co4_im, 4, len_t_theta, nd);
    //print_complex_matrix(sig_co_re, sig_co_im, M, nd);
    //gettimeofday(&time_AWGN_start, NULL);
    cpp_awgn(sig_co_re, sig_co_im, x_r_re, x_r_im, SNR, M, nd);
    cpp_awgn(sig_co4_re, sig_co4_im, x_r4_re, x_r4_im, SNR, 4, nd);
    ...
    float *M_ptr4_im = &M4_im;
    //---------------------------------------------------------------
    complex_matrix_conjugate_transpose_multiplication_single(x_r_re, x_r_im, R_xx_re, R_xx_im, M, nd);
    for (int i = 0; i < M * M; ++i)
    {
        //printf("\t(%f,%f) /= (%f,%f) = ", R_xx_re[i], R_xx_im[i], *M_ptr, *M_ptr_im);
        cpp_division(&R_xx_re[i], &R_xx_im[i], M_ptr, M_ptr_im);
    ...
    complex_matrix_conjugate_transpose_multiplication_single(x_r4_re, x_r4_im, R_xx4_re, R_xx4_im, 4, nd);
    for (int i = 0; i < 4 * 4; ++i)
    {
        //printf("\t(%f,%f) /= (%f,%f) = ", R_xx_re[i], R_xx_im[i], *M_ptr, *M_ptr_im);
        cpp_division(&R_xx4_re[i], &R_xx4_im[i], M_ptr4, M_ptr4_im);
    ...
    //---------------------------------------------------------------
    gettimeofday(&time_Eigen_start, NULL);
    eigen_single(R_xx_re, R_xx_im, Ve_re, Ve_im, De_re, De_im, M, M, qr_iter);
    eigen_single(R_xx4_re, R_xx4_im, Ve4_re, Ve4_im, De4_re, De4_im, 4, 4, qr_iter);
    //gettimeofday(&time_Eigen_end, NULL);
    gettimeofday(&time_Eigen_end, NULL);
    //printf("----------Ve------------\n");
    //print_complex_matrix(Ve_re, Ve_im, M, M);
    ...
    //---------------------------------------------------------------
    gettimeofday(&time_Pn_start, NULL);
    compute_Pn_single(Pn_re, Pn_im, vet_noise_re, vet_noise_im, M, len_t_theta);
    compute_Pn_single(Pn4_re, Pn4_im, vet_noise4_re, vet_noise4_im, 4, len_t_theta);
    //gettimeofday(&time_Pn_end, NULL);
    gettimeofday(&time_Pn_end, NULL);
    //---------------------------------------------------------------
    //printf("----------Pn------------\n");
    ...
    for (int i = 0; i < len_dth; ++i)
    {
        ...
        compute_S_MUSIC_single(a_vector4_re, a_vector4_im, Pn4_re, Pn4_im, 4, &S_MUSIC_re[i], &S_MUSIC_im[i]);
        //printf("\tS_MUSIC(%f,%f), ", S_MUSIC_re[i], S_MUSIC_im[i]);
        S_MUSIC_dB[i] = cpp_20log_abs(&S_MUSIC_re[i], &S_MUSIC_im[i]);
        //printf("S_MUSIC_dB = %.4f\n", S_MUSIC_dB[i]);
        ...
        for (int i = 0; i < len_dthA; ++i)
    {
        ...
        compute_S_MUSIC_single(a_vectorA_re, a_vectorA_im, Pn_re, Pn_im, M, &S_MUSICA_re[i], &S_MUSICA_im[i]);
        //printf("\tS_MUSIC(%f,%f), ", S_MUSIC_re[i], S_MUSIC_im[i]);
        S_MUSICA_dB[i] = cpp_20log_abs(&S_MUSICA_re[i], &S_MUSICA_im[i]);
        //printf("S_MUSIC1_dB = %.4f\n", S_MUSIC1_dB[i]);
    }
...
int main(int argc, char **argv)
{
    ...
    for(int i=126;i<127;i++){
        ...
        //print_code(targetCode,CODE_LENGTH);
        checkpnCode(targetCode, target_sequence,&check_result);
        MUSIC_DOA_2A_single(M, qr_iter, hybrid_qr_iter, &angle[0],number_angle, result, SNR, &check_result);
        //MUSIC_1(argc, argv, &check_result);//後四次
        //printf("\n");
    }
