// SNR_test 測試*127，SNR下限是否正確增加
// g++ -mavx512f -g -o SNR_test SNR_test.c -Wall -Wextra -std=c++14 math_func.a
// ./SNR_test
...
void MUSIC_DOA_2A_CPU_test(int M, int qr_iter,int hybrid_qr_iter, float *angle, int number_angle, float *result, int SNR)
{
    ...
    for (int i = 0; i < len_dth; ++i)
    {
        ...
        //S_MUSIC_re[i]=127*S_MUSIC_re[i]; 
        //S_MUSIC_im[i]=127*S_MUSIC_im[i];
...
int main()
{
    ...
    //-------------------------------------------------------------------
    float angle[100] = {30, 20.1, 50.6, -10, 18, 35, 45, 58}; // angle of array
    int number_angle = 1;
    int M = 64;
    int snr = -25;
    int qr_iter = 1;
