...
//跟m_hybrid_MUSIC_prune差異:細碎時間相加來算演算法時間
//來自於prune_and_search/hybrid_MGS_QR_prune2.c
//當M>8:30度和10度是用M=8的混和MGS-QR做[0.5*lemda] ; 3、1、0.1用float的MGS-QR[0.25*lemda]
//大範圍用compute_pn_i，小範圍用compute_pn
//範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -g -o m_hybrid_MUSIC_prune2 m_hybrid_MUSIC_prune2.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_hybrid_MUSIC_prune2 -t4
...
if(M>8||M==8){  
    ...
    float time_AWGN, time_Eigen, time_Eigen_8=0, time_Pn, time_Pn_8=0, time_search, time_search_8=0, time_small_search, time_vet;                           // create float parameter in order to convert (us) to (ms)
    ...
    */
    ...
    //printf("Total vet time: \t\t%.3f(ms)\n", time_vet / 1000);
    float total_time = time_Eigen_8 + time_Eigen + time_Pn + time_Pn_8 +time_search + time_search_8 + time_small_search + time_vet;
