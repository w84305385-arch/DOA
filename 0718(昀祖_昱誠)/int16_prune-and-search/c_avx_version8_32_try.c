// 都是float //當M>8:30度和10度是用M=8做[0.5*lemda] ; 3、1、0.1用[0.25*lemda]//與c_avx_version8_32差在d，相較之下:d較小會讓峰值較平緩
//範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -g -o c_avx_version8_32_try  c_avx_version8_32_try.c -Wall -Wextra -std=c++14 math_func.a
// ./c_avx_version8_32_try
...
void MUSIC_DOA_2A_CPU_test(int M, int qr_iter, float *angle, int number_angle, float *result, int SNR)
{
    ...
    if(M>8){
        //gettimeofday(&time_Eigen_start, NULL);
        eigen(R_xx_re8, R_xx_im8, Ve_re8, Ve_im8, De_re8, De_im8, 8, 8, qr_iter);
        //gettimeofday(&time_Eigen_end, NULL);
        //printf("----------Ve------------\n");
        //print_complex_matrix(Ve_re, Ve_im, M, M);
        //printf("----------De------------\n");
...
int main()
{
    ...
    //-------------------------------------------------------------------
    float angle[100] = {17, 20.1, 50.6, -10, 18, 35, 45, 58}; // angle of array
    int number_angle = 1;
    int M =64;
    int snr = 40;
    int qr_iter = 10;
