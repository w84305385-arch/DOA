//當M>8:30度和10度是用M=8的混和MGS-QR做[0.5*lemda] ; 3、1、0.1用float的MGS-QR[0.25*lemda]//混合母版本
//範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -g -o hybrid_MGS_QR_prune hybrid_MGS_QR_prune.c -Wall -Wextra -std=c++14 math_func.a
// ./hybrid_MGS_QR_prune
...
void MUSIC_DOA_2A_CPU_test(int M, int qr_iter,int hybrid_qr_iter, float *angle, int number_angle, float *result, int SNR)
{
    ...
    if(M>8){
        //gettimeofday(&time_Pn_start, NULL);
        compute_Pn(Pn_re8, Pn_im8, vet_noise_re8, vet_noise_im8, 8, len_t_theta);
        //gettimeofday(&time_Pn_end, NULL);
        //---------------------------------------------------------------
        //printf("----------Pn------------\n");
