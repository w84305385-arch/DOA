...
void MUSIC_DOA_2A(PHY_VARS_gNB *gNB, int qr_iter, float *angle)
{
    ...
    //-------------------------------------------------------------------
    int M = gNB->rx_doa_param->antenna_num;
    int SNR = gNB->rx_doa_param->snr;
    int number_angle = gNB->rx_doa_param->angle_num;
    float *real_input = gNB->common_vars.rxdataF_real_input; // Real input
    float *imag_input = gNB->common_vars.rxdataF_imag_input; // Imag input
    printf("---------------\n");
    printf("--MUSIC DOA--\n");
    printf("---------------\n");
    printf("--Parameter--\n");
    ...
    */
    ...
    //---------------------------------------------------------------
    //---------------------------------------------------------------
    float *vet_noise_re8 = (float *)malloc(8 * (8 - len_t_theta) * sizeof(float));
    float *vet_noise_im8 = (float *)malloc(8 * (8 - len_t_theta) * sizeof(float));
    //---------------------------------------------------------------
    int16_t *vet_noise_re8_i = (int16_t *)malloc(8 * (8 - len_t_theta) * sizeof(int16_t));
    int16_t *vet_noise_im8_i = (int16_t *)malloc(8 * (8 - len_t_theta) * sizeof(int16_t));
    //---------------------------------------------------------------
    if(M>8 || M==8){
        /*為了用compute_Pn_i關掉
        for (int i = 0; i < 8; ++i)
        {
            for (int j = len_t_theta; j < 8; ++j)
            {
                vet_noise_re8[i * (8 - len_t_theta) + j - len_t_theta] = Ve_re8[i * 8 + j];
                vet_noise_im8[i * (8 - len_t_theta) + j - len_t_theta] = Ve_im8[i * 8 + j];
                //printf("\t(%f,%f)\n", Ve_re[i * M + j], Ve_im[i * M + j]);
            }
        }
        */
        for (int i = 0; i < 8; ++i)
        {
            for (int j = len_t_theta; j < 8; ++j)
            {
                vet_noise_re8_i[i * (8 - len_t_theta) + j - len_t_theta] = Ve_re8[i * 8 + j];
                vet_noise_im8_i[i * (8 - len_t_theta) + j - len_t_theta] = Ve_im8[i * 8 + j];
                //printf("\t(%f,%f)\n", Ve_re8[i * M + j], Ve_im8[i * M + j]);
            }
        }
        //printf("----------vet_noise_i(8)------------\n");
        //print_complex_matrix_i(vet_noise_re8_i, vet_noise_im8_i, 8, (8 - len_t_theta));
    }
    //---------------------------------------------------------------
    //---------------------------------------------------------------
    ...
    //---------------------------------------------------------------
    int16_t *Pn_re8_i = (int16_t *)calloc( 8 * 8, sizeof(int16_t));
    int16_t *Pn_im8_i = (int16_t *)calloc( 8 * 8, sizeof(int16_t));
    //---------------------------------------------------------------
    if(M>8 || M==8){
        //gettimeofday(&time_Pn_start, NULL);
        compute_Pn_i(Pn_re8_i, Pn_im8_i, vet_noise_re8_i, vet_noise_im8_i, 8, len_t_theta);
        //gettimeofday(&time_Pn_end, NULL);
        //---------------------------------------------------------------
        for(int w=0;w<8*8;w++){
        Pn_re8[w]=Pn_re8_i[w];
        ...
        //printf("----------Pn8------------\n");
        //print_complex_matrix(Pn_re8, Pn_im8, 8, 8);
    }
    //---------------------------------------------------------------
    // 取定點
    // parameter setting
...
void ML_DOA_2A(int M, int qr_iter, float *angle, int number_angle, float *result, int SNR) {
    //-------------------------------------------------------------------
...
{
  //-------------------------------------------------------------------
  float *angle = gNB->rx_doa_param->ptr_input_angle; // angle of array
  int number_angle = gNB->rx_doa_param->angle_num;
  int M = gNB->rx_doa_param->antenna_num;
  int snr = gNB->rx_doa_param->snr;
  int qr_iter = gNB->rx_doa_param->qr_iter;
  float result[3] = {0};
  // int angle = 50;
  int iter = 1;

  //=================== MUSIC Algorithm =================================
  if (gNB->MUSIC_enabled != 0) {
    printf(L_RED "\n------------------------------\n");
    printf("AVX512 MUSUC\n");
    printf("------------------------------\n" CLOSE);
    MUSIC_DOA_2A(gNB, qr_iter, gNB->rx_doa_param->ptr_input_angle);
    printf("--------------------------------------\n");
  }
  //=====================================================================

  //=================== MVDR Algorithm ==================================
  if (gNB->MVDR_enabled != 0) {
    printf(L_RED "\n------------------------------\n");
    printf("AVX512 MVDR\n");
    printf("------------------------------\n" CLOSE);
    MVDR_DOA_2A(M, qr_iter, &angle[0], number_angle, result, snr);
    printf("--------------------------------------\n");
  }
  //=====================================================================
  ...
  if (gNB->ML_enabled != 0) {
    ...
    printf("AVX512 ML\n");
    printf("------------------------------\n" CLOSE);
    ML_DOA_2A(M, qr_iter, &angle[0], number_angle, result, snr);
    printf("--------------------------------------\n");
  }
  //=====================================================================
}
