// 都是float //範圍-60~+60度，先以30度找，再以3度找，以0.1度找
// g++ -mavx512f -g -o c_avx_version_prune  c_avx_version_prune.c -Wall -Wextra -std=c++14 math_func.a
// ./c_avx_version_prune
...
void MUSIC_DOA_2A_CPU_test(int M, int qr_iter, float *angle, int number_angle, float *result, int SNR)
{
    ...
    /*
    得轉乘double才能用ceil、floor
    if(floor(t_theta[0]/30)*30 < t_theta[0]< ceil(t_theta[0]/30)*30)  
    {                      
        for (int i = 0; i < len_dth; ++i)
        { // do only one time, no need to be paralleled
          dth[i] = -60 + 30 * i; //-60 -30 0 30 60
    ...
    */
    ...
    {
        //printf("其他position:進else(1)\n");
        for (int i = 0; i < len_dth1; ++i)  
        {  
            dth1[i] = (-39+ (position-1)*30) + 3 * i;
            //printf("dth1:%f\n",dth1[i]);
            dr1[i] = dth1[i] * PI / 180;
        }
    }
    ...
    const int len_dth2 = 61; //0.1度0.1找
    float *dth2 = (float *)malloc(len_dth2 * sizeof(float));
    float *dr2 = (float *)malloc(len_dth2 * sizeof(float));
    ///*
    if(position1 == 0)
    {
        //printf("position1==0:進if(2)\n");
        for (int i = 0; i < len_dth2/2; ++i)  
        {  
            //printf("%d\n",i);
            dth2[i] = -60+0.1*i;
            //printf("dth2:%f\n",dth2[i]);
    ...
    {
        //printf("position == 4:進else if(1)\n");
        for (int i = 0; i < len_dth2; ++i)  
        {  
            dth2[i] = 57 + 0.1 * i;
            //printf("dth2:%f\n",dth2[i]);
            dr2[i] = dth2[i] * PI / 180;
    ...
    if(position1 == 0|| position1==20)
    {
        printf("position1==0:進if(2)\n");
        for (int i = 0; i < len_dth2/2+1; ++i)  
        {  
            //printf("%d\n",i);
            dth2[i] = -60+(position1/2)*3+0.1*i;
            printf("dth2:%f\n",dth2[i]);
            dr2[i] = dth2[i] * PI / 180;
        }
    }
    ...
    //
    //-------------------------------------------------------------------
    float *a_vector2_re = (float *)malloc(M * sizeof(float));
    float *a_vector2_im = (float *)malloc(M * sizeof(float));
    float *S_MUSIC2_re = (float *)malloc(len_dth2 * sizeof(float));
    float *S_MUSIC2_im = (float *)malloc(len_dth2 * sizeof(float));
    float *S_MUSIC2_dB = (float *)malloc(len_dth2 * sizeof(float));
    //---------------------------------------------------------------
    ...
    for (int i = 0; i < len_dth2; ++i)
    {
        ...
        for (int j = 0; j < M; ++j)
        {
            cpp_exp2(&a_vector2_re[j], &a_vector2_im[j], dr2, d, kc, i, j);
            //printf("a_vector(%f,%f)\n", a_vector_re[j], a_vector_im[j]);
        }
        compute_S_MUSIC(a_vector2_re, a_vector2_im, Pn_re, Pn_im, M, &S_MUSIC2_re[i], &S_MUSIC2_im[i]);
        //printf("\tS_MUSIC(%f,%f), ", S_MUSIC_re[i], S_MUSIC_im[i]);
        S_MUSIC2_dB[i] = cpp_20log_abs(&S_MUSIC2_re[i], &S_MUSIC2_im[i]);
        //printf("S_MUSIC1_dB = %.4f\n", S_MUSIC1_dB[i]);
    ...
    //---------------------------------------------------------------
    // find Max and position
    double max_temp2 = S_MUSIC2_dB[0];
    int position2 = 0;
    
    for (int i = 0; i < len_dth2; ++i)
    {
        if (S_MUSIC2_dB[i] > max_temp2)
        {
            max_temp2 = S_MUSIC2_dB[i];
            //printf("max_temp1 = %.4f\n", max_temp1);
            position2 = i;
        }
    }
