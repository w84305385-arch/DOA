...
void complex_matrix_multiplication_iii(int16_t *matA_re, int16_t *matA_im, int16_t *matB_re, int16_t *matB_im, int16_t *matC_re, int16_t *matC_im, int16_t rowA, int16_t rowB, int16_t colB)
{
    ...
    for (int i = 0; i < rowA; i++)
    {
        for (int j = 0; j < colB; j++)
        {
            for (int16_t k = 0; k < ((rowB - 1) / AVX) + 1; k++)
            {
                re_Ai = _mm512_loadu_si512(&matA_re[i * rowB + AVX * k]);
                im_Ai = _mm512_loadu_si512(&matA_im[i * rowB + AVX * k]);
                re_Bi = _mm512_loadu_si512(&matB_re[j * rowB + AVX * k]);
                im_Bi = _mm512_loadu_si512(&matB_im[j * rowB + AVX * k]);
                ...
                _mm512_storeu_si512(&matC_Real_i[(i * colB * rowB + j * rowB) + AVX * k], re_Ci); // store Re value
                _mm512_storeu_si512(&matC_Imag_i[(i * colB * rowB + j * rowB) + AVX * k], im_Ci); // store Im value
            }
        }
    }
