// ML float母版本//當M>8:30度和10度是用M=8 float做[0.25*lemda] ; 3、1、0.1用M float做[0.25*lemda]
// 範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -mavx512bw -g -o float_ML_prune float_ML_prune.c -Wall -Wextra -std=c++14 math_func.a
// ./float_ML_prune
...
void ML_DOA_1D_CPU(int M, int qr_iter, float *angle, int number_angle, float *result, int SNR) {
    ...
    if(M>8){
        ...
        for(int w = 0; w < 8*8 ; w++){
            R_xx_re8[w]=R_xx_re8[w]/2;
            R_xx_im8[w]=R_xx_im8[w]/2;
        }
    }
...
if(M>8){
    ...
    for(int i = 0; i < len_dth; ++i) { 
        ...
        // [inv(A^H*A)]*A^H = 1*M
        complex_matrix_multiplication(AH_mulA_inv_re, AH_mulA_inv_im, a_temp_re, a_temp_im, AH_mulA2_re, AH_mulA2_im, 1, 1, M_re8);
        //printf("(i=%d)a_vector:\n",i);
        //print_complex_matrix(a_vector_re, a_vector_im, 1, M_re8);
        //printf("(i=%d)AH_mulA2:\n",i);
        //print_complex_matrix(AH_mulA_inv_re, AH_mulA_inv_im, 1, M_re8);
        // P_A = A*[inv(A^H*A)]*A^H = M*M  
        complex_matrix_multiplication(a_vector_re, a_vector_im, AH_mulA2_re, AH_mulA2_im,AH_mulA3_re, AH_mulA3_im, M_re8, 1, M_re8);
        //printf("(i=%d)AH_mulA3:\n",i);
        //print_complex_matrix(AH_mulA3_re, AH_mulA3_im, M_re8, M_re8);
        //printf("(i=%d)R_xx:\n",i);
        //print_complex_matrix(R_xx_re8, R_xx_im8, M_re8, M_re8);
        // P_A*R = M*M 
        complex_matrix_multiplication(AH_mulA3_re, AH_mulA3_im, R_xx_re8, R_xx_im8, theta_re, theta_im, M_re8, M_re8, M_re8);
        // trace[P_A*R] 
        trace(theta_re, theta_im, S_ML_re, S_ML_im, M_re8, M_re8, i);
    }
    free(a_temp_re);
    free(a_temp_im);
    for(int i = 0; i < len_dth; ++i) {
...
//*   
    ...
    for(int i = 0; i < len_dthA; ++i) { 
        ...
        //printf("(i=%d)a_temp:\n",i);
        //print_complex_matrix(a_temp_A_re, a_temp_A_im, 1, M_re8);
        // [inv(A^H*A)]*A^H = 1*M
        complex_matrix_multiplication(AH_mulA_inv_A_re, AH_mulA_inv_A_im, a_temp_A_re, a_temp_A_im, AH_mulA2_A_re, AH_mulA2_A_im, 1, 1, M_re8);
        //printf("(i=%d)AH_mulA2:\n",i);
        //print_complex_matrix(AH_mulA_inv_re, AH_mulA_inv_im, 1, M_re8);
        
        // P_A = A*[inv(A^H*A)]*A^H = M*M  
        complex_matrix_multiplication(a_vector_A_re, a_vector_A_im, AH_mulA2_A_re, AH_mulA2_A_im,AH_mulA3_A_re, AH_mulA3_A_im, M_re8, 1, M_re8);
        
        //printf("(i=%d)AH_mulA3:\n",i);
        //print_complex_matrix(AH_mulA3_A_re, AH_mulA3_A_im, M_re8, M_re8);
        // P_A*R = M*M 
        complex_matrix_multiplication(AH_mulA3_A_re, AH_mulA3_A_im, R_xx_re8, R_xx_im8, theta_A_re, theta_A_im, M_re8, M_re8, M_re8);
        //printf("(i=%d)theta_A:\n",i);
        //print_complex_matrix(theta_A_re, theta_A_im, M_re8, M_re8);
        // trace[P_A*R] 
        trace(theta_A_re, theta_A_im, S_ML_A_re, S_ML_A_im, M_re8, M_re8, i);
        //printf("(i=%d)trace完:\n",i);
        //print_complex_matrix(S_ML_A_re, S_ML_A_im, 1, len_dthA);
    }
    free(a_temp_A_re);
    free(a_temp_A_im);
        for(int i = 0; i < len_dthA; ++i) {
