//當M>8:30度和10度是用M=8的混和MGS-QR做[0.5*lemda] ; 3、1、0.1用float的MGS-QR[0.25*lemda]
//和hybrid_MGS_QR_prune.c不同處: 大範圍用compute_pn_i小範圍用compute_pn
//範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -g -o hybrid_MGS_QR_prune2 hybrid_MGS_QR_prune2.c -Wall -Wextra -std=c++14 math_func.a
// ./hybrid_MGS_QR_prune2
...
void MUSIC_DOA_2A_CPU_test(int M, int qr_iter,int hybrid_qr_iter, float *angle, int number_angle, float *result, int SNR)
{
    ...
    */
    ...
    //-------------------------------------------------------------------
    printf("position : \t\t%d\n", position);
    printf(RED "Theta estimation :\t%.5f (degree)\n" CLOSE, dth[position]);
    printf("Max_theta :\t\t%f(dB)\n", max_temp);
    
    printf("positionA : \t\t%d\n", positionA);
    printf(RED "Theta estimationA :\t%.5f (degree)\n" CLOSE, dthA[positionA]);
    printf("Max_thetaA :\t\t%f(dB)\n", max_tempA);
    
    printf("positionB : \t\t%d\n", positionB);
    printf(RED "Theta estimationB :\t%.5f (degree)\n" CLOSE, dthB[positionB]);
    printf("Max_thetaB :\t\t%f(dB)\n", max_tempB);
    
    printf("positionC : \t\t%d\n", positionC);
    printf(RED "Theta estimationC :\t%.5f (degree)\n" CLOSE, dthC[positionC]);
    printf("Max_thetaC :\t\t%f(dB)\n", max_tempC);
    
    printf("positionD : \t\t%d\n", positionD);
    printf(RED "Theta estimationD :\t%.5f (degree)\n" CLOSE, dthD[positionD]);
