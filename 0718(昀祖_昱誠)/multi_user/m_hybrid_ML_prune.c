// ML multi_user混合母版本//當M>8:30度和10度是用M=8 混合做[0.25*lemda] ; 3、1、0.1用M float做[0.25*lemda]
// 範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -mavx512bw -g -o m_hybrid_ML_prune m_hybrid_ML_prune.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_hybrid_ML_prune -t4
...
//-------------------------------------------------------------------
//__attribute__((aligned(32)))int16_t matC_Re_sum_i[30000000] = {0}; // Real
//__attribute__((aligned(32)))int16_t matC_Im_sum_i[30000000] = {0}; // Imaginary
//__attribute__((aligned(32)))int16_t matC_Real_i[30000000] = {0}; // re_C
//__attribute__((aligned(32)))int16_t matC_Imag_i[30000000] = {0}; // im_C
...
/* int16的AVX乘法
void complex_matrix_multiplication_i16(int16_t *matA_re, int16_t *matA_im, int16_t *matB_re, int16_t *matB_im, int16_t *matC_re, int16_t *matC_im, int16_t rowA, int16_t rowB, int16_t colB)
{
    //-------------------------------------------------------------------
    //gettimeofday(&start_transpose, NULL);                        // start
    ...
    // Initialize Global variable array = 0
    //----------------------------------------------------------
    memset(matC_Re_sum_i, 0, rowA * colB * sizeof(int16_t));
    memset(matC_Im_sum_i, 0, rowA * colB * sizeof(int16_t));
    // memset(matA_re, 0, rowA * colB * sizeof(float));
    // memset(matA_im, 0, rowA * colB * sizeof(float));
    // memset(matB_re, 0, rowA * colB * sizeof(float));
    // memset(matB_im, 0, rowA * colB * sizeof(float));
    memset(matC_Real_i, 0, rowA * colB * sizeof(int16_t));
    memset(matC_Imag_i, 0, rowA * colB * sizeof(int16_t));
    //----------------------------------------------------------
    //----------------------------------------------------------

    int colA = rowB;

    __m512i re_Ai, re_Bi, re_Ci; // simd 256 for matrix real part
    __m512i im_Ai, im_Bi, im_Ci; // simd 256 for matrix Imaginary part
    __m512i simd_add, simd_sub;
    memset(matC_re, 0, rowA * colB * sizeof(int16_t)); // Initial matC = 0 (Real)
    memset(matC_im, 0, rowA * colB * sizeof(int16_t)); // Initial matC = 0 (Imaginary)
    //-------------------------------------------------------------------
    for (int i = 0; i < rowA; i++)
    {
        for (int j = 0; j < colB; j++)
        {
            for (int16_t k = 0; k < ((rowB - 1) / AVX16) + 1; k++)
            {
                re_Ai = _mm512_loadu_si512(&matA_re[i * rowB + AVX16 * k]);
                im_Ai = _mm512_loadu_si512(&matA_im[i * rowB + AVX16 * k]);
                re_Bi = _mm512_loadu_si512(&matB_re[j * rowB + AVX16 * k]);
                im_Bi = _mm512_loadu_si512(&matB_im[j * rowB + AVX16 * k]);
                re_Ci = _mm512_sub_epi16(_mm512_mullo_epi16(re_Ai, re_Bi), _mm512_mullo_epi16(im_Ai, im_Bi)); // Re{ac-bd}
                im_Ci = _mm512_add_epi16(_mm512_mullo_epi16(re_Ai, im_Bi), _mm512_mullo_epi16(im_Ai, re_Bi)); // Im{ad+bc}

                _mm512_storeu_si512(&matC_Realf[(i * colB * rowB + j * rowB) + AVX16 * k], re_Ci); // store Re value
                _mm512_storeu_si512(&matC_Imagf[(i * colB * rowB + j * rowB) + AVX16 * k], im_Ci); // store Im value
            }
        }
    }
    //gettimeofday(&end_multiplyf, NULL);
    //  timersub(&end_multiply, &start_multiply, &diff_multiply); // calculate
    //  printf(L_PURPLE "\nElapsed AVX512 multiply time: %ld(us)\n" CLOSE, (long int)diff_multiply.tv_usec);
    ...
    for (int i = 0; i < rowA * colB; i++)
    {
        ...
        for (int a = 0; a < rowB; a++)
        {
            matC_Re_sum_i[i] += matC_Real_i[i * rowB + a];
            matC_Im_sum_i[i] += matC_Imag_i[i * rowB + a];
            // printf("matC_Re_sum[%d] = %.2f, ", i, matC_Re_sum[i]);
            //    printf("matC_Im_sum[%d] = %.0f, ", i * colB + a, matC_Im_sum[i]);
        }
        // printf(" \n");
    ...
    for (int i = 0; i < rowA * colB; i++)
    {
        // matC[i] = {matC_Real[i], matC_Imag[i]};
        //  matC[i] = matC_Imag[i];
        matC_re[i] = matC_Re_sum_i[i];
        matC_im[i] = matC_Im_sum_i[i];
    }
    //-------------------------------------------------------------------
    //gettimeofday(&start_transpose, NULL);           // start
    ...
    //-------------------------------------------------------------------
    //timersub(&end_transpose, &start_transpose, &diff_transpose); // calculate total transpose time
    //otal_pre_transpose_time += diff_transpose.tv_usec;          // global variable can store transpose time
    //------------------------------------------------------------
}
...
void trace_i(int16_t *theta_re, int16_t *theta_im, int16_t *S_ML_re, int16_t *S_ML_im, int rowA, int colA, int i){
    int16_t temp_re = 0.0 ;
    int16_t temp_im = 0.0 ;
    for(int id=0; id < colA ;id++){
        temp_re += theta_re[id *colA+id];
...
void *ML_DOA_1D_CPU(void *struct_var) 
{
    //-------------------------------------------------------------------
    ...
    int16_t *R_xx_re8_i = (int16_t *)malloc(8 * 8 * sizeof(int16_t));
    int16_t *R_xx_im8_i = (int16_t *)malloc(8 * 8 * sizeof(int16_t));
    float M_re8 = 8;
    float M_im8 = 0.0;
    float *M_ptr8 = &M_re8;
...
if(M>8){
    ...
    //-------------------------------------------------------------------
    int16_t *a_vector_re_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *a_vector_im_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));
    int16_t *a_temp_re_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *a_temp_im_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *AH_mulA_inv_re_i = (int16_t *)malloc(1 * 1 * sizeof(int16_t));  
    int16_t *AH_mulA_inv_im_i = (int16_t *)malloc(1 * 1 * sizeof(int16_t));  
    int16_t *AH_mulA2_re_i = (int16_t *)malloc(1 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA2_im_i = (int16_t *)malloc(1 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA3_re_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA3_im_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *theta_re_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *theta_im_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *S_ML_re_i = (int16_t *)malloc(len_dth * sizeof(int16_t));
    int16_t *S_ML_im_i = (int16_t *)malloc(len_dth * sizeof(int16_t));
    //-------------------------------------------------------------------
    pthread_mutex_lock(&mutex);
    for(int i = 0; i < len_dth; ++i) { 
...
//*   
    ...
    //-------------------------------------------------------------------
    //-------------------------------------------------------------------
    int16_t *a_vector_A_re_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *a_vector_A_im_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));
    int16_t *a_temp_A_re_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *a_temp_A_im_i = (int16_t *)malloc(M_re8 * sizeof(int16_t));  
    int16_t *AH_mulA_inv_A_re_i = (int16_t *)malloc(1 * 1 * sizeof(int16_t));  
    int16_t *AH_mulA_inv_A_im_i = (int16_t *)malloc(1 * 1 * sizeof(int16_t));  
    int16_t *AH_mulA2_A_re_i = (int16_t *)malloc(1 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA2_A_im_i = (int16_t *)malloc(1 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA3_A_re_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *AH_mulA3_A_im_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *theta_A_re_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *theta_A_im_i = (int16_t *)malloc(M_re8 * M_re8 * sizeof(int16_t));
    int16_t *S_ML_A_re_i = (int16_t *)malloc(len_dth * sizeof(int16_t));
    int16_t *S_ML_A_im_i = (int16_t *)malloc(len_dth * sizeof(int16_t));
    //-------------------------------------------------------------------
    pthread_mutex_lock(&mutex);
    for(int i = 0; i < len_dthA; ++i) { 
        for(int j = 0; j < M_re8; ++j) {
...
int main(int argc, char **argv)
{
    ...
    float timeML_end[8]= {0.0}; // Total ML Algorithm time
    //-------------------------------------------------------------------
    ...
    for (int i = 0; i < thread_num; i++)
    {
        ...
        if (pthread_create(&th[i], NULL, &ML_DOA_1D_CPU, ml_param) != 0)
        {
...
/*
    ...
    free(AH_mulA3_A_re);
    free(AH_mulA3_A_im);
    free(AH_mulA_inv_A_re);
    free(AH_mulA_inv_A_im);
*/
