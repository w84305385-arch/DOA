//和m_hybrid_ML_prune.c不同處:+/-、轉置、共軛轉置、get_row、get_colum用thread
// ML multi_user混合母版本//當M>8:30度和10度是用M=8 混合做[0.25*lemda] ; 3、1、0.1用M float做[0.25*lemda]
// 範圍-60~+60度，30->10->3->1->0.1 
// g++ -mavx512f -mavx512bw -g -o m_hybrid_ML_prune2 m_hybrid_ML_prune2.c -Wall -Wextra -std=c++14 math_func.a -lpthread
// ./m_hybrid_ML_prune2 -t4
...
//*   
    ...
    // timersub function
    //-------------------------------------------------------------------
    float time_AWGN, time_search, time_search_8, time_small_search;                           // create float parameter in order to convert (us) to (ms)
    ...
    */
    ...
    float total_time = time_search +time_search_8+ time_small_search  ; 
